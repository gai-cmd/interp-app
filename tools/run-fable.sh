#!/bin/bash
# Drive a task list with Claude Fable (owner rule 2026-09-06: Astra designs/reviews, Fable codes), one `codex exec` per task, verifying with node --test
# and committing on success. Prints one line per task (Monitor-friendly). Exit 2 = task failed
# twice (hand off to Fable), exit 3 = Astra blocked (rate/usage limit).
shopt -s nullglob
APP="$HOME/work/interp-app"; cd "$APP" || exit 1
GITC="git -c user.name=gai -c user.email=gai@kaflixcloud.co.jp"
run_tests() {
  local files=(tests/*.test.mjs)
  [ ${#files[@]} -eq 0 ] && { echo "no tests"; return 0; }
  node --test "${files[@]}" > docs/build/last-test.log 2>&1
}
blocked() { grep -qiE "usage limit|rate limit|429|too many requests|overloaded|not logged in|Invalid API key|nested" "$1"; }
PREFIX="${1:-P1}"
HOMES=("fable")   # single lane · Claude subscription
for f in docs/build/tasks/$PREFIX-*.md; do
  id=$(basename "$f" .md)
  case "$id" in *.fable|*.retry) continue;; esac
  [ -f "docs/build/$id.done" ] && continue
  t0=$(date +%s)
  ok=0
  for home in "${HOMES[@]}"; do
    tools/fable-task.sh "$id" "$f" >/dev/null
    if blocked "docs/build/$id.log"; then echo "$id limited on $(basename "$home") · trying next profile"; continue; fi
    ok=1; break
  done
  if [ $ok -eq 0 ]; then echo "$id FABLE-BLOCKED on all profiles $(grep -iE 'usage limit|try again' docs/build/$id.log | head -1 | cut -c1-140)"; exit 3; fi
  if run_tests; then
    $GITC add -A >/dev/null; $GITC commit -qm "$id: $(grep -m1 '^## ' "$f" | sed 's/^## //' | cut -c1-60)

Built by Claude Fable 5.1; verified with node --test.
Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>
Claude-Session: https://claude.ai/code/session_01LuvfLUrfz3d8VPU49Ce22F" >/dev/null 2>&1
    touch "docs/build/$id.done"; echo "$id OK $(( $(date +%s) - t0 ))s files=$($GITC show --stat --format= HEAD | tail -1)"
    continue
  fi
  # retry once with the failure attached
  { cat "$f"; echo; echo "# 이전 시도 실패 · 아래 테스트 출력을 보고 고쳐라 (같은 규칙 · 같은 파일 범위)"; echo '```'; tail -80 docs/build/last-test.log; echo '```'; } > "docs/build/tasks/$id.retry.md"
  ok=0
  for home in "${HOMES[@]}"; do
    tools/fable-task.sh "$id-retry" "docs/build/tasks/$id.retry.md" >/dev/null
    if blocked "docs/build/$id-retry.log"; then continue; fi
    ok=1; break
  done
  if [ $ok -eq 0 ]; then echo "$id FABLE-BLOCKED on retry (all profiles)"; exit 3; fi
  if run_tests; then
    $GITC add -A >/dev/null; $GITC commit -qm "$id (retry): $(grep -m1 '^## ' "$f" | sed 's/^## //' | cut -c1-60)

Built by Claude Fable 5.1 (second attempt); verified with node --test.
Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>
Claude-Session: https://claude.ai/code/session_01LuvfLUrfz3d8VPU49Ce22F" >/dev/null 2>&1
    touch "docs/build/$id.done"; echo "$id OK-after-retry $(( $(date +%s) - t0 ))s"
    continue
  fi
  echo "$id FAILED-twice → handoff · $(grep -cE '^not ok' docs/build/last-test.log) failing tests"; exit 2
done
echo "$PREFIX COMPLETE"
