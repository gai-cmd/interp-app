...
| 2026-09-07 · P3-47 (Vercel) | p3-47-20260907 | **Vercel** `gai-cmds-projects/interp-app` (CLI 직접 업로드, 배포 루트 `~/deploy/interp-app`) | https://interp-app.vercel.app | 소스 `94847b4` · 1006 테스트 · `stage-release --builtin-key-file` → check-release `RELEASE_BUILTIN_KEY` → `RELEASE_OK releases=1 files=106` · `vercel-json.mjs` rules=8 · 배포 후 curl: `/` 200 + **CSP·Permissions-Policy·nosniff·no-referrer·DENY 실제 적용**, `sw.js`/`/` no-cache, `policy.json` no-store, `releases/*` immutable, worklet·manifest·i18n MIME 정확, 키 슬롯 채워짐 | 새 키(무료) 주입 · 이후 기본 호스트 · 롤백: `--point p3-46`은 이 루트에 없으므로 재스테이징 |
| 2026-09-07 · P3-47 (Pages) | p3-47-20260907 | GitHub Pages (`gh-pages` `75d8b03`) | https://gai-cmd.github.io/interp-app/ | 소스 `94847b4` · check-release RELEASE_OK releases=14 · 배포 후 curl SW `p3-47`, 키 슬롯 비어 있음 확인 | **키 없는 빌드**(개인 키 입력 안내). 첫 키는 이 호스트로 push된 직후 GitHub 시크릿 스캐닝(alert #1, 01:21Z)→구글 무효화로 죽었음 · 롤백 `p3-46-20260907` |
| 2026-09-07 · P3-48/49 (Vercel) | p3-48-20260907, p3-49-20260907 (+콘솔 배포 `keys-20260907-0254`) | Vercel `interp-app` | https://interp-app.vercel.app | 소스 `e368f75`…`0242dac` · 1010 테스트 · check-release RELEASE_BUILTIN_KEY→OK · **실브라우저(headless Chrome, 가짜 마이크) 시작→번역 전용 모델 연결됨 15초 유지·폴백 없음 확인** | 번역 전용 모델 설정 회귀(P3-40, transcription 위치) 수정 · 죽은 폴백 모델 교체 · 무료 키 교대(4개) · 헤더 업데이트 버튼 · 롤백 `p3-47-20260907` |
| 2026-09-07 · P3-49 (Pages, 폐쇄) | p3-49-20260907 | GitHub Pages (`gh-pages` `c55ad18`) | https://gai-cmd.github.io/interp-app/ | check-release RELEASE_OK releases=16 · policy.json rev2 | **호스트 폐쇄**: `emergency.stopped=true`(이유에 새 주소) + critical 공지 `moved-to-vercel`. 키 없는 빌드. 되돌리기: policy rev1 + `--point p3-48` |
| 2026-09-24 · P3-52 (Vercel) | p3-52-20260924 | Vercel `interp-app` (`dpl_GWLmSpfvY1mR3AaRx25PD3bHk6HK`) | https://interp-app.vercel.app | 소스 `9d254ca` · 1011 테스트 · check-release RELEASE_BUILTIN_KEY→`RELEASE_OK current=p3-52-20260924 releases=8 files=743` · vercel-json rules=8 · 배포 후 curl: `/` 200 + CSP·Permissions-Policy·nosniff·no-referrer·DENY, `/`·`sw.js` no-cache, `policy.json` no-store, `releases/*` immutable, index가 p3-52를 가리킴 · **실브라우저(headless Chrome, 가짜 마이크) 시작→"범용 Live 모델 · gemini-3.8-live" 연결됨 18초 유지·폴백 없음** | 동시통역 기본 모델을 `gemini-3.8-live`로(오너 결정) · 레거시 `gemini-3.1-flash-live-preview` 제거, Live Translate는 첫 폴백 · 미리듣기 모델도 3.8 Live · 롤백: `--point p3-51-20260907` |
