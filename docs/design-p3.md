...
**P3-41 범위 결정(2026-09-07):** 이번 P3 화면은 정책 단가만 사용한다. 개인 단가 입력은 제거하고 정책이 허용할 때에도 미지원 안내만 표시한다. 단일 숫자로 모델·능력·통화를 지정할 수 없기 때문이다. 개인 단가 저장 위치는 만들지 않으며 숫자·범위·통화 입력을 받지 않는다. `allowLocalOverride`가 변경되어도 개인 단가를 읽거나 적용하지 않고 안내 표시만 갱신한다. 기존 정책 요율 revision 고정·추정 불가 규칙은 유지한다. 개인 입력은 항목별 검증과 저장 계약을 별도 설계한 뒤 도입한다.
...
## 1.11.1 P3-40 — 동시통역 언어 선택 최종 결정

2026-09-07 확정. 동시통역의 언어 의미에 관해서는 이전 구현 설명보다 이 절을 우선한다.

- **단방향의 도착 언어는 모든 통역 결과의 언어다.** 발화가 자동 감지이면 모델이 인식할 수 있는 다른 언어의 말을 도착 언어로 통역한다. 이미 도착 언어인 말, 자기 출력, 비발화 소음은 읽지 않는다. 모든 언어의 인식 정확도를 보장한다는 뜻은 아니다.
- 발화 언어를 명시해도 **배타적 필터가 아니라 예상 발화 언어에 대한 참고 선택**이다. 다른 언어를 무시한다고 약속하지 않는다. 지시문 경로는 예상 언어와 다른 언어도 같은 도착 언어로 통역하도록 명시한다. 번역 전용 경로는 발화 힌트를 전달할 필드가 없으므로 자동 감지한다. 두 경로 모두 도착 언어 고정·다른 발화 언어 허용이라는 동작 계약은 같다.
- `translationConfig.sourceLanguageCode`는 사용하지 않는다. [공식 Live Translate 설정](https://ai.google.dev/gemini-api/docs/live-api/live-translate)의 필드는 `targetLanguageCode`와 `echoTargetLanguage`다. 번역 전용 모델의 자막 설정은 `generationConfig` 아래에 놓고, 일반 Live 모델은 기존 최상위 자막 설정을 유지한다. 명시 발화 언어로 영어 오출력이 반드시 해결된다는 설명은 철회한다.
- **양방향은 화면의 두 선택 `[발화, 도착]` 사이를 양쪽으로 통역한다.** 두 선택은 서로 다른 실제 언어여야 한다. 자동 감지는 양방향에서 선택할 수 없다. 자동 상태에서 양방향을 켜면 도착 언어를 유지하고 `ko → ja → en` 우선순위에서 다른 언어를 발화에 채운 뒤, 기존 쌍 표시로 알린다. 따라서 도착이 한국어이면 일본어가 채워지고 영어가 암묵적으로 끼어들지 않는다. 제3언어에는 침묵하도록 지시한다. 단일 세션·마이크·목소리를 공유하며 번역 전용 모델에서는 등록된 일반 Live 모델로 전환한다.
- 같은 언어를 선택하면 마지막으로 선택한 쪽을 유지하고 반대쪽을 위 우선순위의 다른 언어로 이동한다. 화면의 두 선택을 실제 요청에 그대로 사용한다. 교환은 실제 언어 두 개일 때만 허용한다. 발화 자동 감지에서 교환은 비활성화한다.
- 발화·도착 선택 변경은 기존 연결을 종료하고 다음 수동 시작에 적용한다. 기존 양방향 토글은 진행 중 연결을 유지하며 재시작 안내를 표시하고, 다음 시작/세션 다시 열기에 적용한다. 이미 연결된 모델의 지시문을 실시간 변경했다고 표시하지 않는다.
- 단방향 자동 상태에만 `sim.sourceAuto`의 자동 감지 의미를 표시한다. 명시 상태에는 `sim.sourceHint`로 참고 선택·번역 모델의 자동 감지를 설명한다. 양방향에서는 기존 쌍·양방향 안내를 표시하고 이 두 힌트는 숨긴다. 허브 수신에는 발화 선택이 없다.
- UI 언어와 통역 언어는 계속 별개다. 저장 키 `interp-app.ui.v1.simSource`, `interp-app.ui.v1.twoWay`는 유지하며 새로 비밀이나 원문을 저장하지 않는다.
- 모델 발견 결과의 순서는 최신성이나 설정 호환성의 증거가 아니다. 미등록 모델로 자동 전환하지 않는다. 발견 목록은 유지하되 실행 어댑터에 등록되지 않은 모델은 `MODEL_UNSUPPORTED`로 거부한다. 임의 이름으로 설정 경로를 추정해 실행 지원을 확대하지 않는다.


## P3-44 — 첫 방문 기본 키와 홈 이동 최종 결정 (2026-09-07)

설정 이력이 없는 기기는 내장 키로 동시통역을 시작할 수 있어야 한다. `d6310f8`·`4c96219`의 방향은 수용하되, 설정에서 타인의 키를 개인 키처럼 표시·삭제하게 하는 동작과 홈 이동의 중지·포커스·겹침 결함을 수정했다. 오너가 수용한 평문 배포와 단일 파일 비밀 검사 예외는 유지한다.

### 키 표시와 수명

대안 (a)를 선택한다. 설정과 배지에 **사이트 기본 키**를 별도로 표시하고, 내장 키는 입력란에 마스크나 원문을 채우지 않으며 표시 토글과 삭제를 허용하지 않는다. 사용자가 새로 입력하는 개인 키의 표시 토글은 유지한다. §1.12의 자동 재노출 금지에 맞고, 사용자가 넣은 적 없는 키를 자기 소유로 오인하지 않게 한다.

새로운 라우팅 출처 `builtin`은 만들지 않는다. 기존 `personal` 인증 경로와 정책·Live 계약을 유지하고 비밀 없는 메타데이터의 `builtin: true`로 표시만 구분한다. 내장 키 길이는 0, `revealPersonal()`은 null을 반환한다. 모델 탐색은 UI 재노출 함수를 쓰지 않고 기존 자격증명 참조 경계를 통해 인증한다. 모델 탐색 실패는 시작을 막지 않는다.

- 부팅 시 저장된 개인 키와 수신한 공용 키가 내장 키보다 우선한다. 공용 키 선택은 기존의 명시 선택 계약을 유지한다.
- 기본 키는 브라우저 저장소에 접근하지 않고 메모리에 등록한다. 저장소 차단·미지원도 기본 키 등록을 막지 않는다.
- 개인 키를 입력하면 기본 키를 대체한다. 그 개인 키를 삭제하면 기본 키로 복귀하고 저장된 개인 키를 지운다. 삭제 이벤트는 기존 참조를 무효화하고 활성 통역을 정리한다. 자동 재시작하지 않는다.
- 공용 키가 남아 있으면 개인 키 삭제로 기본 키를 자동 설치하지 않는다. 공용 모드의 종료·만료도 개인/기본 키로 자동 전환하지 않는다. 기본 키 자체의 삭제 요청은 무효다.
- 내장 키 없는 빌드의 키 없음 안내·삭제 동작은 유지한다.
- `rememberPersonalKey: false`는 개인 키 신규 저장을 막고 기본 키의 메모리 사용을 막지 않는다. 실제 앱의 설정 화면에 정책·개인 설정 저장소 연결이 빠져 있던 것도 연결했다. 정책 조회 중 저장 선택을 영구 해제하지 않으며 정책 확정 후 금지를 적용한다.
- `sharedKeys: false`는 기본 키의 개인 인증 경로와 충돌하지 않는다. 긴급 중지·직접 통역 금지·정책 조회 실패는 그대로 시작을 차단한다.

### 요금제와 비밀 경계

요금제 섹션은 기본 키 사용 시 운영자 계정으로 청구된다는 안내를 표시한다. Free/Paid는 이 기기의 사용량 표시 방식이며 실제 무료 이용이나 결제 활성화를 의미하지 않는다. 기존 개인 경로 사용량 집계를 유지하며, 전체 운영자 계정의 사용량으로 표시하지 않는다. 개인 키 교체·삭제 시 안내와 배지를 갱신한다.

내장 키는 `app/security/builtin-key.js`에서만 관리한다. DOM·입력값·페이지 URL·이력·오류·콘솔·사용량·진단·사이트 공유 QR·정책 내보내기에 복사하지 않는다. 기존 브라우저 WebSocket 인증 URL과 REST/모델 탐색 인증 헤더는 필요한 자격증명 전달 경계다. 따라서 “모든 URL에 키가 없음”을 주장하지 않는다. 특히 Live 인증 URL은 기존 구현상 키를 포함한다. 시험은 합성 키만 사용하며 실제 내장 키를 시험·문서로 옮기지 않는다.

### 홈과 접근성

홈은 열린 시트 닫기 → 자막 전용 화면 나가기 → 동시통역 탭 선택 → 해당 탭 버튼 포커스로 통일한다. 일반 탭 전환의 `stopWork`를 호출하지 않으므로 진행 중 통역은 유지한다. 이전 비동기 탭 이동도 무효화해 늦은 완료가 홈 선택을 덮지 못하게 한다.

설정·화면·공유 시트에는 자체 홈 버튼을 제공한다. 시트가 열린 동안 배경 헤더는 `inert`지만 내부 홈은 포커스 트랩 안에서 사용 가능하며, 닫은 뒤 배경의 inert를 해제하고 동시통역 탭으로 포커스를 보낸다.

자막 홈과 최신 자막 버튼을 상단바 밖의 일반 흐름 하단 행에 넣었다. flex 줄바꿈과 축소 금지로 번역 라벨이 길어도 서로 겹치지 않고, 중앙 시작 영역과 별도 공간을 차지한다. 공통 버튼의 최소 너비·높이 44px와 표면·글자 토큰을 그대로 사용하므로 라이트·다크·mono 색상에 따른다. 세 언어 라벨은 홈 / Home / ホーム, 접근성 이름은 메인 화면으로 / Back to the main screen / メイン画面へ를 유지한다. 렌더링·실기기 관찰 없이 코드·CSS로 검수한 범위다.
