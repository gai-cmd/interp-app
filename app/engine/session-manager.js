...
export function createSessionManager({ timeoutMs = 10000, ...timing } = {}) {
  ...
  async function shutdown(entry) {
    ...
    if (!entry.closing) {
      ...
      }).then(() => { if (current === entry) current = null; });
      entry.closing.catch(() => {});
    }
  ...
  return Object.freeze({
    ...
    replace(open, context = {}) {
      ...
      return serialize(async () => {
        ...
        const onEvent = (event) => {
          ...
          if (event?.type === 'closed') { shutdown(entry).catch(() => {}); }
          ...
          for (const method of ['speak', 'cancel', 'sendAudio', 'finishInput']) {
            ...
            lease[method] = (...args) => {
              ...
              return withDeadline(() => session[method](...args), { ...timing, timeoutMs, signal: controller.signal })
                .catch((error) => { shutdown(entry).catch(() => {}); throw normalizeError(error); });
            };
          }
    ...
    close() {
      ...
      if (current) { current.controller.abort(); current.detach(); }
