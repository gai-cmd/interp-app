...
test('P3-15 header: title, badges, then KO EN JA · display · share · settings in DOM order; the tab row sits under the header on mobile', () => {
  ...
  assert.deepEqual(classNames(app), ['shell-header', 'shell-notice', 'shell-message', 'shell-first-run', 'shell-tabs', 'shell-main', 'shell-settings sheet', 'shell-display sheet', 'share-dialog sheet']);
...
test('P3-15 desktop header: the 64rem query moves the tab row between the title and the badges, and back, keeping focus', () => {
  ...
  const app = h.root.childNodes[0];
  const header = h.shell.elements.header;
  const stacked = ['shell-header', 'shell-notice', 'shell-message', 'shell-first-run', 'shell-tabs', 'shell-main', 'shell-settings sheet', 'shell-display sheet', 'share-dialog sheet'];
